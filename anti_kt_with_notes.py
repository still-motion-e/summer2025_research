'''
the anti-kt paper:
    https://arxiv.org/pdf/0802.1189
the kt paper:
    https://arxiv.org/pdf/hep-ph/9305266
'''

#to run this code type the following into the command line -> python3 anti_kt_with_notes.py Eta_Phi_PT_Mass_Objects.txt 
...
def min_finder(value,cone):
    #this will hold the smallest distance between two particles
    #it is intially set to infinity
    smallest_distance = float('inf')
    #parent_j & parent_i are the two particles that make that smallest distance
    parent_i = 0
    parent_j = 0
    #this will be smallest beam distance
    #it is intially set to infinity
    smallest_PT2 = float('inf')
    #this is the parent object that measured the smallest beam distance
    smallest_PT2_parent = 0
    
    #this first for loop will run to the for the length of the first term given to the function
    for j in range(value):
        #this is if statment compares the PT^-2 to the previous pt^-2 and checks if it is smaller
        if data['PT'].loc[data.index[j]]**-2 < smallest_PT2:
            #if the condition of the if statement is true, it sets puts the new smallest value in the variable location "smallest_PT2"
            smallest_PT2 = data['PT'].loc[data.index[j]]**-2
            #this puts the parent that makes the smallest beam distance in "smallest_PT2_parent"
            smallest_PT2_parent = j
        #this nested for loop allows us to loop through all the possible combinations when looking for the smallest distance between two particles 
        for i in range(j):
            #this builds the delta Phi
            d_phi = np.abs(data['Phi'].loc[data.index[i]]-data['Phi'].loc[data.index[j]])
            #this builds the delta Eta
            d_eta = data['Eta'].loc[data.index[i]]-data['Eta'].loc[data.index[j]]
            
            #this if statment will handle the 2Pi problem and correct for that
            if d_phi > np.pi:
                d_phi = 2*np.pi - d_phi
            #this builds the R value for any two particles
            r_value = (d_phi**2+d_eta**2)
            #this builds the distance for any two particles
            distance_d = min(math.pow(data['PT'].loc[data.index[i]],-2),math.pow(data['PT'].loc[data.index[j]],-2))*(r_value)/cone**2
            #this if statement will determine if the new distance is smaller than the previous distance 
            if distance_d < smallest_distance:
                smallest_distance = distance_d
                parent_j = j
                parent_i = i
    return smallest_distance, parent_i, parent_j, smallest_PT2, smallest_PT2_parent
...
while merge == True:
    if len(data) == 1:
        ...
        for item in merge_history:
            print(item)
        break
    d_value, parent_i, parent_j, smallest_PT2, pt2_parent = min_finder(len(data),0.4)
